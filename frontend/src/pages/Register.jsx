...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success("Welcome to EcoMind 🌱");
      nav("/dashboard");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col relative overflow-hidden noise">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-cyan-500/10 blur-[120px] pointer-events-none" />

      <nav className="relative z-10 max-w-6xl mx-auto w-full px-6 py-6">
        <Link to="/" className="inline-flex items-center gap-2.5" data-testid="register-logo-link">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-6 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="glass-strong rounded-3xl p-8 border border-white/5">
            <h1 className="font-display font-bold text-3xl tracking-tight mb-2">Join EcoMind</h1>
            <p className="text-zinc-500 text-sm mb-8">Start saving carbon with every prompt.</p>

            <button
              onClick={loginWithGoogle}
              data-testid="google-signup-btn"
              className="w-full flex items-center justify-center gap-3 py-3 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 text-white text-sm font-medium transition-colors mb-6"
            >
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                <label className="text-xs text-zinc-500 uppercase tracking-widest mb-2 block">Full name</label>
            ...
            <div className="mt-6 text-center text-sm text-zinc-500">
              ...
              <Link to="/login" className="text-emerald-400 hover:text-emerald-300" data-testid="register-goto-login">
                Sign in
              </Link>
            </div>
          </div>
